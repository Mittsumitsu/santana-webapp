  ...
  try {
    ...
    if (!location || location === 'any' || location === '') {
      return res.status(400).json({
        error: 'ご利用になる店舗を選択してください',
        message: 'デリー、バラナシ、プリーのいずれかの店舗を選択してください。',
      });
    }
    ...
    const roomsByLocation = allRooms.filter(room => room.location_id === location);
    ...
    console.log(`🎯 【空室チェック付き】検索完了: ${enhancedCombinations.length}パターン生成`);
    ...
    console.error('🚨 【空室チェック付き】検索エラー:', error);
